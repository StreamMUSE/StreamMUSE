...
class BulkBenchmarkRunner:
    ...
    def _create_experiment_name(
        self, params: Dict[str, Any], experiment_name: str = None
    ) -> str:
    ...
    def _run_single_experiment(
        ...
        for key in ["tempo", "assumed_network_latency_ms", "inference_interval_ticks"]:
            if (
                key in self.config["benchmark"]
                and self.config["benchmark"][key] is not None
            ):
        ...
        if injection_config.get("enabled", False):
            ...
            if "prompt_length_ticks" in params and injection_file_path:
                ...
                print(
                    f"🎵 Will inject {params['prompt_length_ticks']} ticks from {injection_file_path}"
                )
        ...
        try:
            ...
            if not output_file.exists():
                raise FileNotFoundError(
                    f"Benchmark output file not created: {output_file}"
                )
    ...
    def _generate_basic_statistics(self, df: pd.DataFrame, output_dir: Path) -> None:
        ...
        with open(stats_file, "w") as f:
            ...
            if "round_trip_time" in df.columns:
                ...
                # Percentiles
                percentiles = self.config.get("analysis", {}).get(
            ...
            param_columns = [
                ...
                if col in ["generation_length_frames", "prompt_length_ticks", "tempo"]
            ]
    ...
    def run_experiments(self) -> None:
        ...
        main_combinations = self._generate_experiment_combinations(
        ...
        if "additional_experiments" in self.config:
            for add_exp in self.config["additional_experiments"]:
                combinations = self._generate_experiment_combinations(
                    add_exp["parameters"]
                )
        ...
        experiment_progress = tqdm(
            all_experiments, desc="Running Experiments", unit="exp"
        )
...
Examples:
  ...
  %(prog)s experiments/my_experiment.yaml --dry-run
    ...
    if args.dry_run:
        ...
        main_combinations = runner._generate_experiment_combinations(
        ...
        if "additional_experiments" in runner.config:
            for add_exp in runner.config["additional_experiments"]:
                combinations = runner._generate_experiment_combinations(
                    add_exp["parameters"]
                )
        ...
        if "additional_experiments" in runner.config:
            for add_exp in runner.config["additional_experiments"]:
                total += len(
                    runner._generate_experiment_combinations(add_exp["parameters"])
                )
