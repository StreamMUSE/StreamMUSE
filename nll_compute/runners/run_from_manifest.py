...
from nll_compute.batch import make_nll_dir
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        "--device", type=str, default=None, help="Device string like 'cuda:0' or 'cpu'. If omitted, auto-selects."
    )
    parser.add_argument(
        "--model_size",
        type=str,
        default="0.12B",
        help="Model size string passed to load_from_checkpoint (e.g. '0.12B')",
    )
    parser.add_argument(
        "--process-root-as-dir",
        action="store_true",
        help="If set, call make_nll_dir on the manifest root path itself instead of per-subdirectory",
    )
        ...
        if args.process_root_as_dir:
            ...
            try:
                ...
                if isinstance(data, dict) and len(data) == 0:
                    print(
                        f"  Combined JSON at {save_path} contains 0 files — falling back to per-subdirectory processing"
                    )
