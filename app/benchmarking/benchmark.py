...
def inject_music_to_server(
    server_url: str, injection_file_path: str, injection_length_ticks: int
):
    """
    ...
    try:
        ...
        if result["success"]:
            print(
                f"✅ Injection successful: {result['melody_notes_injected']} melody notes, "
                f"{result['accompaniment_notes_injected']} accompaniment notes"
            )
...
def run_benchmark(
    ...
    tempo: float = None,
    assumed_network_latency_ms: float = None,
    inference_interval_ticks: int = None,
    prompt_length_ticks: int = None,
    injection_file_path: str = None,
    injection_length_ticks: int = None,
):
    """
    ...
    melody_notes = [
        {"pitch": 64, "tick": 0, "duration": 4},
        {"pitch": 62, "tick": 4, "duration": 4},
        {"pitch": 60, "tick": 8, "duration": 4},
        {"pitch": 62, "tick": 12, "duration": 4},
    ]
    ...
    if injection_file_path and injection_length_ticks:
        injection_offset_ticks = inject_music_to_server(
            server_url, injection_file_path, injection_length_ticks
        )
        if injection_offset_ticks == 0:
            print("Injection failed. Halting benchmark.")
            return
        print(
            f"🎯 Injection offset: {injection_offset_ticks} ticks. Adjusting generation_start_tick."
        )
    ...
    for i in tqdm(range(num_requests), desc="Benchmarking", unit="req"):
        ...
        try:
            ...
            preprocess_duration = (
                timings["inference_start_time"] - timings["preprocess_start_time"]
            )
            inference_duration = (
                timings["inference_end_time"] - timings["inference_start_time"]
            )
            postprocess_duration = (
                timings["response_output_time"] - timings["postprocess_start_time"]
            )
    ...
    print(
        f"Generated Notes per Request: {statistics.mean(num_notes):.1f} "
        f"(std: {statistics.stdev(num_notes):.1f})"
    )
    print()
    print("Timing Breakdown:")
    print(
        "  Round Trip Time:       "
        f"{statistics.mean(round_trips)*1000:.1f}ms "
        f"(std: {statistics.stdev(round_trips)*1000:.1f}ms)"
    )
    print(
        "  Server Processing:     "
        f"{statistics.mean(server_processing)*1000:.1f}ms "
        f"(std: {statistics.stdev(server_processing)*1000:.1f}ms)"
    )
    print(
        "    - Preprocessing:     "
        f"{statistics.mean(preprocess_times)*1000:.1f}ms "
        f"(std: {statistics.stdev(preprocess_times)*1000:.1f}ms)"
    )
    print(
        "    - Inference:         "
        f"{statistics.mean(inference_times)*1000:.1f}ms "
        f"(std: {statistics.stdev(inference_times)*1000:.1f}ms)"
    )
    print(
        "    - Postprocessing:    "
        f"{statistics.mean(postprocess_times)*1000:.1f}ms "
        f"(std: {statistics.stdev(postprocess_times)*1000:.1f}ms)"
    )
    print(
        "  Network Latency:       "
        f"{statistics.mean(network_latencies)*1000:.1f}ms "
        f"(std: {statistics.stdev(network_latencies)*1000:.1f}ms)"
    )
...
if __name__ == "__main__":
    ...
    run_benchmark(
        ...
        args.tempo,
        args.assumed_network_latency_ms,
        args.inference_interval_ticks,
        args.prompt_length_ticks,
        args.injection_file_path,
        args.injection_length_ticks,
    )
